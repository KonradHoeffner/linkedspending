...
@Log @SuppressWarnings("serial") public class HttpConnectionUtil
{
	...
	@AllArgsConstructor public static class HttpException extends Exception
	{
	...
	public static class HttpUnavailableException extends HttpException
	{
		public HttpUnavailableException(URL url)
		{
			super(url);
		}
	}
	...
	public static final HttpURLConnection getConnection(URL url) throws InterruptedException, IOException, HttpTimeoutException,
			HttpUnavailableException
	{
		int retry = 0;
		...
		throw new HttpTimeoutException(url); // we retried at least once but we didn't return yet so
												// it has to be a timeout exception
	}
}
