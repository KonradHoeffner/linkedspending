...
@NonNullByDefault @Log public class JsonDownloader extends OpenspendingSoftwareModule implements Runnable
{
	...
	public boolean getDownloadStopped()
	{
	...
	{
		return Paths.get(pathJson.getPath(), datasetName).toFile();
	}
	...
	protected static void downloadAll() throws JsonProcessingException, IOException, InterruptedException, ExecutionException
	{
		...
		if (TEST_MODE != null)
		{
			datasetNames = new TreeSet<>(Collections.singleton(TEST_MODE));
		}
		else
		{
	...
	public static void main(String[] args) throws JsonProcessingException, IOException, InterruptedException, ExecutionException
	{
		...
		// try{LogManager.getLogManager().readConfiguration();log.setLevel(Level.FINER);} catch (
