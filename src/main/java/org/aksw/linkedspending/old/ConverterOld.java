...
import org.aksw.linkedspending.convert.ComponentProperty;
import org.aksw.linkedspending.convert.Countries;
import org.aksw.linkedspending.convert.ResultsReader;
import org.aksw.linkedspending.convert.ComponentProperty.Type;
...
@Log @NonNullByDefault public class ConverterOld extends OpenspendingSoftwareModule implements Runnable
{
	...
	static
	{
		try (TSVReader in = new TSVReader(ConverterOld.class.getClassLoader().getResourceAsStream("codetocurrency.tsv")))
		{
	...
	static
	{
		try (TSVReader in = new TSVReader(ConverterOld.class.getClassLoader().getResourceAsStream("propertymapping.tsv")))
		{
			while (in.hasNextTokens())
			{
				String[] tokens = in.nextTokens();
